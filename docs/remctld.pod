...
tab-separated strings, where each line is:
    ...
    type service executable [logmask=n[,...]] {acl}+
...
used as mentioned below under I<service>, a set of commands).  The first
configuration line matching the received command is used, so list more
specific entries before more general entries.
...
thought of as the sub-command.  If the keyword C<ALL> is used instead of a
specific service, this line matches all commands with the given type and
can be used to dispatch all services under that type to the same
executable with the same ACLs.  Since the first matching entry is used,
list entries for specific services first (if any) and then the C<ALL>
catch-all.

B<remctld> accepts commands with no service argument, only a type.  Such
commands will only match lines in the configuration file with C<ALL> for
the service.  (The B<remctl> client will not send such commands, but they
can be sent using the client library or via another implementation.)
