...
so the program run will not be able to distinguish between different
commands.
...
expressions. Can be used in chain with deny:pcre:regexp to deny access based
