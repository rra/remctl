...
subcommand (or, if C<ALL> is used as mentioned below under I<service>, a
...
the release function of the AFS volume backend.  If the keyword C<ALL> is
...
and then the C<ALL> catch-all.

B<remctld> accepts commands with no subcommand argument.  Such commands
will only match lines in the configuration file with C<ALL> for the
service.  (The B<remctl> client will not send such commands, but they can
...
The full path to the commmand executable to run for this command and
...
The I<subcommand> cannot be passed on standard input, so I<n> must be at
least C<2>.  If this option is set to C<last> and no arguments are given
...
not fit on the command line.  It can also be used for arguments like
passwords that shouldn't be exposed on the command line.  Only at most one
argument may be passed on standard input to the command.
...
C<deny:deny:foo> neither deniess nor grants access to anyone.
