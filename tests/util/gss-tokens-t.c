...
main(void)
{
    ...
    ok_int(2, 3, token_flags);
    ok(3, token_length > 5);
    server_tok.value = token_buffer;
    server_tok.length = token_length;
    ...
    token_length = server_tok.length;
    memcpy(token_buffer, server_tok.value, server_tok.length);
    gss_release_buffer(&c_min_stat, &server_tok);
