...
public class RemctlResponse {
     ...
     */
    public static RemctlResponse buildFromTokens(final Iterable<RemctlToken> responseTokens) {
        ...
        for (RemctlToken outputToken : responseTokens) {
            ...
            if (outputToken instanceof RemctlErrorToken) {
                ...
                if (stream == 1) {
                    ...
                    throw new RemctlErrorException(RemctlErrorCode.ERROR_BAD_TOKEN.value, "Unrecognized stream "
                            + stream);
                }
            } else if (outputToken instanceof RemctlStatusToken) {
                status = ((RemctlStatusToken) outputToken).getStatus();
                break;
            } else {
