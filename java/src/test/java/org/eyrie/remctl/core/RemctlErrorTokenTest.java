...
public class RemctlErrorTokenTest {
	...
	public void testFromBytes() {
		...
		// -- test no message --
		byte[] message = new byte[] { 0, 0, 0, 1, /* error code */
		0, 0, 0, 0 /* no message */
		};
		...
		// -- test with message --
		message = new byte[] { 0, 0, 0, 4, /* error code */
		...
		assertEquals("Error code should match",
				RemctlErrorCode.ERROR_BAD_COMMAND, errorToken.getCode());
		assertEquals("Message not expected", "Boo", errorToken.getMessage());
	}
	...
	public void testUnexpectedErrorToken() {
		// -- test no message --
		byte[] message = new byte[] { 1, 0, 0, 0, /* error code 4096 */
		0, 0, 0, 0, /* no message */
		};
	...
	public void testBadMessageLength() {
		...
		byte[] message = new byte[] { 0, 0, 0, 8, /* error code 8 */
		...
		// --- not enough bytes ---
		message = new byte[] { 0, 0, 0, 3, /* error code 8 */
		0, 0, 0 /* missing byte */
		};
		try {
			new RemctlErrorToken(message);
