...
public class RemctlServer extends Remctl {
	...
	public interface Servlet {
		public int run(RemctlServer server, String[] args) throws GSSException,
				IOException;
	}
	...
	/* make a server */
	public RemctlServer(Socket p_socket, GSSCredential serverCreds)
	...
	public int make_error(int code, String s) throws GSSException, IOException {
		if (returnCode != 2)
			return -2;
	...
	public void make_output(byte where, String s) throws GSSException,
			IOException {
		if (returnCode != 2)
			return;
		byte[] bytes = s.getBytes();
	...
	public void make_out(String s) throws GSSException, IOException {
		make_output((byte) 1, s);
	}

	public void make_err(String s) throws GSSException, IOException {
		make_output((byte) 2, s);
	}

	// XXX big. can this be broken up?
	public void serve_a_client(Servlet servlet) throws GSSException,
			IOException {
		byte state = TOKEN_V2_INIT;
		client_loop: while (keptalive || returnCode != 0) {
			outStream.flush();
			...
			/*
			 * cs old rc new rc 0 0 2 1 0 1 2 1 1 3 1 2
			 */
