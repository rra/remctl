...
public class RemctlClient extends Remctl {
	...
	/**
	 * Factory method that creates a <code>LogincContext</code> and returns a
	 * RemctlClient that executed from within that context.
	 * 
	 * @param args
	 *            array of args to pass to remctl server
	 * @param host
	 *            host name of remctl server.
	 * @param port
	 *            port of remctl server. If 0, uses the default port of 4373
	 *            (Remctl.DEFAULT_PORT).
	 * @param princ
	 *            Principal to use. If null, uses the default principal of
	 *            <code>"host/"+host</code>.
	 * @param lcName
	 *            Name passed to the <code>LoginContext</code> constructor. If
	 *            null, use the default name of "Remctl".
	 */

	// XXX this uses jaas. can bcs be done as well somehow?
	public static RemctlClient withLogin(final String args[],
			final String host, final int port, final String princ,
			final String lcName)
			throws javax.security.auth.login.LoginException,
			java.security.PrivilegedActionException {
		final LoginContext lc = new LoginContext(lcName != null ? lcName
				: DEFAULT_NAME,
		...
		PrivilegedExceptionAction pea = new PrivilegedExceptionAction() {
			public Object run() throws GSSException, IOException {
				return new RemctlClient(args, host, port, princ, System.out,
						System.err);
			}
		};

		try {
			return (RemctlClient) Subject.doAsPrivileged(lc.getSubject(), pea,
					null);
		} finally {
			lc.logout();
		}
	...
	/**
	 * Factory method that creates a <code>LogincContext</code> and returns a
	 * RemctlClient that executed from within that context. Uses default values
	 * for port/principal/name.
	 * 
	 * @param args
	 *            array of args to pass to remctl server
	 * @param host
	 *            host name of remctl server. If null, use the default name of
	 *            "RemctlClient".
	 */
	public static RemctlClient withLogin(final String args[], final String host)
			throws javax.security.auth.login.LoginException,
			java.security.PrivilegedActionException {
	...
	/**
	 * Invoke Remctl on the specified host, with the specified args. If no
	 * exceptions are thrown, you would then normally check the return code with
	 * <code>getReturnCode()</code>, and the returned message with
	 * <code>getReturnMessage()</code>.
	 * <p>
	 * Should only be called within a login context that has the proper Kerberos
	 * credentials. The factory methods <code>withLogin</code> can be used when
	 * a single RemctlCLient object is needed. If multiple RemctlClient objects
	 * are needed, then the application should itself set up the login context,
	 * invoke Subject.doAsPrivileged, and then create multiple RemctlObjects
	 ...
	 *            <code>"host/"+host</code>.
	 * @param p_out
	 *            Writer to receive stdout from remote job.
	 * @param p_err
	 *            Writer to receive stderr from remote job.
	 */
	public RemctlClient(String args[], String host, int port,
			String servicePrincipal, Writer p_out, Writer p_err)
	...
	public RemctlClient(String args[], String host, int port,
			String servicePrincipal, PrintStream p_out, PrintStream p_err)
			throws GSSException, IOException {
		this(args, host, port, servicePrincipal, new PrintWriter(p_out, true),
				new PrintWriter(p_err, true));
	}

	public RemctlClient(String args[], String host, int port,
			String servicePrincipal) throws GSSException, IOException {
		this(args, host, port, servicePrincipal, System.out, System.err);
	}

	public RemctlClient(String host, int port, String servicePrincipal,
			Writer p_out, Writer p_err) throws GSSException, IOException {
	...
	public RemctlClient(String host, int port, String servicePrincipal,
			Writer p_out, Writer p_err, final LoginContext lc)
			throws GSSException, IOException {
		this(host, port, servicePrincipal, p_out, p_err);
		PrivilegedExceptionAction pea = new PrivilegedExceptionAction() {
			public Object run() throws GSSException, IOException {
	...
	public void process(boolean keepalive, String args[]) throws GSSException,
			IOException {
		if (!keptalive) {
			returnCode = -1;
			return;
	...
	public void process(String args[]) throws GSSException, IOException {
		this.process(true, args);
	}

	public void finishup() throws GSSException, IOException {
		processQuit();
		context.dispose();
		out.flush();
		err.flush();
	...
	public void clientEstablishContext() throws GSSException, IOException {
		...
		/*
		 * Create a GSSName out of the service name. The null indicates that
	...
	private void processRequest(ArrayList byteArgs, boolean keepalive)
		...
		while (messageBuffer.hasRemaining()) {
			...
			if (messageBuffer.position() != 0)
				count += 4;
			if (count > TOKEN_MAX_DATA)
				count = TOKEN_MAX_DATA;
	...
	private byte processResponse() throws GSSException, IOException {
		...
		if (returnType != MESSAGE_V2)
			throw new IOException("remctl: Message protocol version was "
					+ returnType + " not 2?");
		returnType = messageBuffer.get();
	...
	public static void main(String[] args) {
		...
		try {
			...
			RemctlClient rc = RemctlClient.withLogin(remargs, host, 0, null,
					null);
