...
remctl [B<-p> I<port>] [B<-s> I<service>] [B<-v> I<verbose>] I<host>
I<type> I<service> [I<parameters> ...]
...
Access to remote commands is authenticated via Kerberos v5 over GSSAPI, so
a user must have a ticket granting ticket to use B<remctl>.  All
transmissions to and from the remctld server are encrypted using GSSAPI's
security layer.
