...
configured (see below).  B<remctld> logs its activity using syslog (the
daemon facility).

The location of the configuration file may be specified with the B<-f>
option.  The default is F<@sysconfdir@/remctl.conf>.
...
allowed commands and specifying access control information.  The
configuration file format is lines of space or tab separated strings,
where each line is:
