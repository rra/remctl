...
remctld [B<-dhmv>] [B<-f> I<config>] [B<-p> I<port>] [B<-s> I<service>]
...
4444 by default, and handle incoming connections one at a time.  This is
...
Kerberos v5 principal per line, such as C<username@stanford.edu>.
...
principal from the REMUSER environment variable.
