                         ...
                         OM_uint32 *ret_flags)
{
    ...
    if (token_send(WRITEFD, TOKEN_NOOP | TOKEN_CONTEXT_NEXT, empty_token) < 0) {
    ...
    *gss_context = GSS_C_NO_CONTEXT;
    ...
    do {
        ...
        if (send_tok.length != 0) {
            ...
            if (token_send(WRITEFD, TOKEN_CONTEXT, &send_tok) < 0) {
        ...
        if (maj_stat == GSS_S_CONTINUE_NEEDED) {
            ...
            if (token_recv(READFD, &token_flags, &recv_tok, MAXENCRYPT) < 0) {
